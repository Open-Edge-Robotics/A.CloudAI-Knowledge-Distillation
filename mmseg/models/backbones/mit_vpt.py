...
from .mit import MixFFN, EfficientMultiheadAttention, MixVisionTransformer
...
class MixVisionTransformer_vpt(MixVisionTransformer):
    def __init__(self, in_channels=3, embed_dims=64, num_stages=4, num_layers=[3, 4, 6, 3], num_heads=[1, 2, 4, 8], patch_sizes=[7, 3, 3, 3], strides=[4, 2, 2, 2], sr_ratios=[8, 4, 2, 1], out_indices=(0, 1, 2, 3), mlp_ratio=4, qkv_bias=True, drop_rate=0, attn_drop_rate=0, drop_path_rate=0, act_cfg=dict(type='GELU'), norm_cfg=dict(type='LN', eps=0.000001), pretrained=None, init_cfg=None, with_cp=False):
        ...
        self.vpt_embeddings = nn.ParameterList(  # batch, num_prompt, embed_dim
            [nn.Parameter(torch.empty(1, self.vpt_num,  embed_dims * num_heads[i])) for i in
                range(num_layers)])
        for eee in self.vpt_embeddings:
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            for block in layer[1]:
                eee = self.vpt_embeddings[i].expand(x.size(0), -1, -1)
                print(x.shape, eee.shape)
