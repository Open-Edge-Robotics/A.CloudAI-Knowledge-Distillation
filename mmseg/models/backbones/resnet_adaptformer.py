...
class BasicBlock_adaptformer(BaseModule):
    ...
    def __init__(self,
        ...
        down_size = 64
        self.adaptformer_down_proj = nn.Conv2d(planes, down_size, 1, 1, 0, bias=False)
        self.non_linear_func = nn.ReLU()
        self.adaptformer_up_proj = nn.Conv2d(down_size, planes, 1, 1, 0, bias=False)
        # self.adaptformer_layer_norm_before = nn.LayerNorm(inplanes)
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            ...
            out_adapt = self.forward_adapt(out)
            out = self.conv2(out)
            out = self.norm2(out)
...
class ResNet_adaptformer(BaseModule):
    """ResNet backbone.
    ...
    arch_settings = {
        18: (BasicBlock_adaptformer, (2, 2, 2, 2)),
        34: (BasicBlock_adaptformer, (3, 4, 6, 3)),
        ...
        if isinstance(pretrained, str):
            ...
            if init_cfg is None:
                ...
                if self.zero_init_residual:
                    if block is BasicBlock_adaptformer:
                        block_init_cfg = dict(
                            type='Constant',
                            val=0,
...
@MODELS.register_module()
class ResNetV1c_adaptformer(ResNet_adaptformer):
