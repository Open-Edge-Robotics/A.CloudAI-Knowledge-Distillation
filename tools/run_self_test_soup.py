...
def main():
    ...
    # state_dict = torch.load('./work_dirs/rein_dinov2l_mask2former_1024x1024_bs4x2/iter_40000.pth')
    checkpoint = load_checkpoint(model, './work_dirs/rein_dinov2l_mask2former_1024x1024_bs4x2/iter_40000.pth', map_location='cpu')
    state_dict1 = copy.deepcopy(model.state_dict())

    checkpoint = load_checkpoint(model, './work_dirs/rein_dinov2l_mask2former_1024x1024_bs4x2_30000/iter_80000.pth', map_location='cpu')
    ...
    # if args.backbone:
    checkpoint = load_checkpoint(model.backbone, './checkpoints/dinov2l_converted_1024x1024.pth', map_location='cpu')


    test_soup = {k : v.clone() * 1/2. + state_dict1[k] * 1/2. for k, v in state_dict2.items()}
    model.load_state_dict(test_soup, strict=False)
