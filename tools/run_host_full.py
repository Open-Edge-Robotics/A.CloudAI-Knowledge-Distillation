...
class CloudManager():
    ...
    def run_distillation_pl(self, image_array):
        ...
        # loss = F.cross_entropy(outputs, resize(foutputs_, self.resize_shape_cloud, mode='bilinear')).mean()
        loss.backward()
        self.optimizer.step()        
        dis_end = time.time()
...
def main():
    ...
    cloud_manager = CloudManager(
        host='172.27.183.203', port=9999, 
        dl_host='172.27.183.202', dl_port=9998,
